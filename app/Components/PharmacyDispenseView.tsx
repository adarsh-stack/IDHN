...
import autoTable from 'jspdf-autotable'; // PATCH 1: Explicitly import autoTable
...
export default function PharmacyDispenseView({ inventory, onRefresh }: PharmacyDispenseViewProps) {
  ...
  const addToBasket = (item: MedicineItem) => {
    if (item.stockCount <= 0) return alert('Cannot dispense out-of-stock items.');
    const existing = basket.find(b => b._id === item._id);
    if (existing) {
      if (existing.dispenseQty >= item.stockCount) return alert('Maximum available stock exceeded.');
      setBasket(basket.map(b => b._id === item._id ? { ...b, dispenseQty: b.dispenseQty + 1 } : b));
    } else {
  ...
  const updateBasketQty = (id: string, qty: number, maxStock: number) => {
    ...
    if (qty <= 0) {
      ...
      setBasket(basket.map(b => b._id === id ? { ...b, dispenseQty: qty } : b));
    }
  };

  const grandTotal = basket.reduce((acc, b) => acc + (b.dispenseQty * b.pricePerUnit), 0);
  ...
  const generatePDFInvoice = (billId: string) => {
    ...
    // Billee Info
    doc.text(`Billed To: ${billToPatient ? `Patient ID: ${patientId}` : billeeName}`, 14, 45);
    if (!billToPatient) doc.text(`Phone: ${billeePhone}`, 14, 50);
    doc.text(`Payment Status: ${paymentStatus.toUpperCase()} (${paymentMethod})`, 14, 55);

    // Table
    const tableColumn = ["Medicine / Item", "Batch", "Qty", "Unit Rate", "Total"];
  ...
  const handleCheckoutDispense = async () => {
    if (basket.length === 0) return alert('Your dispensing basket is completely empty.');
    if (billToPatient && !patientId) return alert('Please enter a valid Patient ID or switch to manual billee.');
    if (!billToPatient && !billeeName) return alert('Please provide the external person\'s name for the bill.');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 relative">
      ...
      {successModal.show && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[3000]">
          <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full text-center space-y-4 animate-in zoom-in-95 duration-200 border border-[#eadecc]">
            ...
            <div>
              <h3 className="text-xl font-bold font-serif text-[#0e1e38]">Transaction Complete</h3>
      ...
      <div className="lg:col-span-3 bg-white border border-[#eadecc] rounded-2xl shadow-sm p-5 space-y-3 h-fit">
        <h3 className="text-sm font-bold font-serif text-[#0e1e38] mb-2">Available Therapeutics Catalog</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {inventory.map((item) => (
            <div key={item._id} className="p-4 bg-[#fcfaf4] border border-[#eadecc]/60 rounded-xl flex justify-between items-center shadow-sm">
              <div>
                <h4 className="font-bold text-xs text-[#0e1e38]">{item.name}</h4>
                <p className="text-[10px] text-gray-400 font-medium">Stock: {item.stockCount} units • Rate: ₹{item.pricePerUnit.toFixed(2)}</p>
              </div>
              <button 
      ...
      <div className="lg:col-span-2 bg-white border border-[#eadecc] rounded-2xl p-6 shadow-sm flex flex-col justify-between h-fit space-y-6">
        <div>
          <h3 className="text-sm font-bold font-serif text-[#0e1e38] border-b border-gray-150 pb-2.5 mb-4">Dispensing Summary Basket</h3>
          {basket.length === 0 ? (
            <p className="text-xs text-gray-400 py-6 text-center">No medications staged for verification.</p>
          ) : (
            <div className="space-y-3 divide-y divide-gray-100/60 max-h-48 overflow-y-auto pr-1">
              {basket.map((b) => (
                <div key={b._id} className="flex justify-between items-center pt-2.5 first:pt-0 text-xs">
                  <div className="space-y-0.5">
                    <p className="font-bold text-[#0e1e38]">{b.name}</p>
                    <p className="text-[10px] text-gray-400 font-mono">₹{(b.pricePerUnit * b.dispenseQty).toFixed(2)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <input 
                      type="number" 
                      value={b.dispenseQty} 
                      onChange={(e) => updateBasketQty(b._id!, Number(e.target.value), b.stockCount)}
                      className="w-12 bg-slate-50 border border-gray-200 rounded p-1 text-center font-mono text-xs focus:outline-none"
                    />
                    <button onClick={() => updateBasketQty(b._id!, 0, b.stockCount)} className="text-red-400 hover:text-red-600 font-bold px-1 text-sm cursor-pointer">✕</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="border-t border-gray-150 pt-4 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {paymentMethod === 'UPI' && grandTotal > 0 && (
              <div className="border border-dashed border-[#eadecc] p-1.5 rounded-lg bg-white shadow-sm flex flex-col items-center">
                <img src={qrCodeImage} alt="Paytm/UPI QR" className="w-16 h-16 object-contain" />
                <span className="text-[8px] font-bold text-gray-400 mt-1 uppercase tracking-widest">Scan & Pay</span>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-between items-center text-sm font-bold text-[#0e1e38] bg-[#fcfaf4] p-3 rounded-xl border border-[#eadecc]/60">
            ...
            <span className="font-mono text-base text-emerald-600">₹{grandTotal.toFixed(2)}</span>
          </div>
          ...
          >
            {isProcessing ? 'Generating PDF Invoice...' : 'Execute Dispense & Print Invoice'}
          </button>
        </div>
      </div>
    </div>
