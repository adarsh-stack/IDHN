...
export async function updateDoctorStatus(
  ...
  try {
    ...
    const result = await db
      .collection("users")
      .updateOne({ _id: new ObjectId(doctorId) }, { $set: updatePayload });
...
export async function fetchAvailableDoctors(): Promise<{
  success: boolean;
  data: DoctorSelectOption[];
}> {
  try {
    const { db } = await connectToDatabase();
...
export async function fetchDoctorsGroupedByExpertise(): Promise<{
  success: boolean;
  data: GroupedDoctors[];
}> {
  try {
    ...
    const result = (await db
      .collection("users")
      .aggregate(pipeline)
      .toArray()) as unknown as GroupedDoctors[];
